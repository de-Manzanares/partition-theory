...
\section{Exercises from Lecture Notes For An Introductory Minicourse on q-Series}\label{sec:lecture}

\paragraph{1.1} Verify the inversion identity.
...
\subparagraph{(\textit{i})}\label{subpar:1.1(i)}
\begin{align*}
...
\subparagraph{(\textit{ii})}\label{subpar:1.1(ii)}
\begin{align*}
...
\subparagraph{(\textit{iii})}\label{subpar:1.1(iii)}
\begin{align*}
    (aq^n;q)_k &= \frac{(a;q)_n(aq^n;q)_k}{(a;q)_n} = \frac{(a;q)_k(aq^k;q)_n}{(a;q)_n} \\[0.5em]
\end{align*}

\subparagraph{(\textit{iv})}\label{subpar:1.1(iv)}
...
\section{Exercises from \emph{Basic Hypergeometric Series}}\label{sec:bhs}

\paragraph{1.1}
Verify the identities (1.2.30)-(1.2.40); $k$ and $n$ are integers.

\subparagraph{(1.2.30)}
In the case that $n$ is a positive integer, See exercise 1.1(\textit{i}) on~\autopageref{subpar:1.1(i)}.
...
\subparagraph{(1.2.33)}
For $n,k\in\mathbb{Z}_+$, See exercise 1.1(\textit{ii}) on~\autopageref{subpar:1.1(ii)}.
...
\subparagraph{(1.2.34)}
See exercise 1.1(\textit{iii}) on~\autopageref{subpar:1.1(iii)}.
...
\subparagraph{(1.2.38)}
See exercise 1.1(\textit{iv}) on~\autopageref{subpar:1.1(iv)}.
